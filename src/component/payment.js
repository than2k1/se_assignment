...
const Payment = (prop) => {
  ...
  // console.log(myCard)
  //'total' use to calculate total price of cart
  let total = 0;
          ...
          {myCard.map((item) => {
            return (
              <ListPay
                key={item.id}
                itemCard={item}
                deleteItem={prop.delete}
                increase={prop.increase}
                decrease={prop.decrease}
              />
            );
          })}
        </div>
        <div className="total">
          <div id="paymentInfo">
            ...
            <label>
              <h2>
                Kr {total.toFixed(2)}
                <span style={{ fontSize: "70%", display: "inline" }}>$</span>
              </h2>
